...
public class PerDAO implements PerDAO_interface {
	...
	private static final String INSERT = 
			"INSERT INTO PERMISSION  VALUES ('P'||LPAD(to_char(PER_SEQ.NEXTVAL), 9, '0'),?)";
		private static final String GET_ALL = 
			"SELECT FROM PERMISSION order by PER_ID";
		private static final String GET_ONE= 
			"SELECT FROM PERMISSION where PER_ID = ?";
		private static final String DELETE = 
			"DELETE FROM PERMISSION where PER_ID = ?";
		private static final String UPDATE = 
			"UPDATE PERMISSION set PER_NAME=?";
	...
	public void inser(PerVO perVo) {
		...
		try {
			...
			pstmt.setString(1, perVo.getPER_NAME ());
	...
	public PerVO findByPrimaryKey(String PER_ID) {
		...
		try {
			...
			while (rs.next()) {
				...
				perVO = new PerVO();
				perVO.setPER_NAME(rs.getString("PER_NAME"));
	...
	public List<PerVO> getAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				list.add(perVO); // Store the row in the list
			}
